import { readFileSync, readdirSync, statSync } from "node:fs";
import { join } from "node:path";
import Link from "next/link";
import matter from "gray-matter";
import { ContractFilterGrid, type ContractRow } from "@/components/contract/ContractFilterGrid";

const COMPONENT_DIR = join(process.cwd(), "contract", "components");

function readComponentRows(): ContractRow[] {
  const rows: ContractRow[] = [];
  try {
    for (const entry of readdirSync(COMPONENT_DIR)) {
      if (!entry.endsWith(".mdx")) continue;
      const full = join(COMPONENT_DIR, entry);
      if (statSync(full).isDirectory()) continue;
      const { data: fm } = matter(readFileSync(full, "utf8"));
      if (!fm.component) continue;
      rows.push({
        slug:       entry.replace(".mdx", ""),
        name:       fm.component as string,
        type:       "component",
        status:     (fm.parity as string) ?? "unknown",
        path:       fm.path as string | undefined,
        usageCount: (fm["usage-count-30d"] as number | null | undefined) ?? null,
      });
    }
  } catch {}
  return rows.sort((a, b) => a.name.localeCompare(b.name));
}

export default function ComponentsIndexPage() {
  const rows = readComponentRows();
  const drifted = rows.filter(r => r.status === "drifted").length;

  return (
    <div className="min-h-screen bg-background text-foreground">
      <div className="max-w-3xl mx-auto px-6 py-12">
        {/* Nav */}
        <nav className="flex gap-6 mb-10 text-[11px] font-mono text-muted-foreground">
          <Link href="/contract" className="hover:text-foreground transition-colors">
            ← contract
          </Link>
          <Link href="/contract/tokens" className="hover:text-foreground transition-colors">
            tokens →
          </Link>
        </nav>

        {/* Header */}
        <div className="mb-8">
          <p className="text-[11px] font-mono text-muted-foreground uppercase tracking-widest mb-2">
            Contract Index
          </p>
          <h1 className="text-2xl font-mono font-bold">Components</h1>
          <div className="flex gap-4 mt-2 text-[12px] font-mono text-muted-foreground">
            <span>{rows.length} total</span>
            {drifted > 0 && <span className="text-yellow-400/80">{drifted} drifted</span>}
          </div>
        </div>

        <ContractFilterGrid rows={rows} type="component" />
      </div>
    </div>
  );
}
